document.addEventListener('DOMContentLoaded', () => {
    ...
    const refineriesList = [
        ...
        { id: 'ufaneftekhim', name: { ru: 'Башнефть-Уфанефтехим', uk: 'Башнафта-Уфанафтохім', en: 'Bashneft-Ufaneftekhim' }, company: 'Башнефть / Роснефть', region: { ru: 'Башкортостан, Уфа', uk: 'Башкортостан, Уфа', en: 'Bashkortostan, Ufa' }, capacity: 12.0, query: 'уфанефтехим', lat: 54.9333, lng: 56.0667 },
    ...
    const npzGrid = document.getElementById('npzGrid');
    const npzFloatingTooltip = document.getElementById('npzFloatingTooltip');
    ...
    function showNpzTooltip(e, ref) {
        ...
        npzFloatingTooltip.innerHTML = content;
        npzFloatingTooltip.classList.remove('opacity-0', 'invisible');
        npzFloatingTooltip.classList.add('opacity-100', 'visible');
    ...
    function updateNpzTooltipPosition(e) {
        ...
        const tWidth = npzFloatingTooltip.offsetWidth || 230;
        const tHeight = npzFloatingTooltip.offsetHeight || 140;
        ...
        npzFloatingTooltip.style.left = `${left}px`;
        npzFloatingTooltip.style.top = `${top}px`;
    }

    function hideNpzTooltip() {
        if (!npzFloatingTooltip) return;
        npzFloatingTooltip.classList.add('opacity-0', 'invisible');
        npzFloatingTooltip.classList.remove('opacity-100', 'visible');
    }
    ...
    function openNpzDrawer() {
        ...
        if (npzDrawer) {
            npzDrawer.classList.remove('-translate-x-full');
            npzDrawer.classList.add('translate-x-0');
        }
        if (npzDrawerBackdrop) {
            npzDrawerBackdrop.classList.remove('opacity-0', 'pointer-events-none');
            npzDrawerBackdrop.classList.add('opacity-100', 'pointer-events-auto');
        }
        renderNpzDrawer();
    ...
    function closeNpzDrawer() {
        ...
        if (npzDrawer) {
            npzDrawer.classList.remove('translate-x-0');
            npzDrawer.classList.add('-translate-x-full');
        }
        if (npzDrawerBackdrop) {
            npzDrawerBackdrop.classList.add('opacity-0', 'pointer-events-none');
            npzDrawerBackdrop.classList.remove('opacity-100', 'pointer-events-auto');
        }
        hideNpzTooltip();
    }
